...
| ✅ 已完成 | 🟡 P2 | 4 | `main.js` 回调注入 → store/event 模式 | 4-6h | 架构安全，杜绝遗漏注入 |
| ✅ 已完成 | 🟡 P2 | 5 | `scraper.py` 解析器拆分（1395行） | 3-4h | 可测试性，降低改动风险 |
| ✅ 已完成 | 🟡 P2 | 6 | `strategy.py` 计算逻辑提取到 services | 2-3h | 可测试，可复用 |
| ✅ 已完成 | 🟡 P2 | 7 | 补充 MA60 / rebalance / amounts 单元测试 | 4-5h | 防止回归 |
| ✅ 已完成 | 🟡 P2 | 8 | `advice.js` 渲染与逻辑分离 | 2-3h | 逻辑可测试 |
| ✅ 已完成 | 🟢 P3 | 9 | 总金额标签更新函数提取 | 30min | 去重 |
| ✅ 已完成 | 🟢 P3 | 10 | journal 扫描逻辑提取为共享函数 | 30min | 去重 |
| ✅ 已完成 | 🟢 P3 | 11 | SSE 重连机制 | 1h | 健壮性 |
| ✅ 已完成 | 🟢 P3 | 12 | `_rowSnapshots` 清理 | 15min | 防内存泄漏 |
| ✅ 已完成 | 🟢 P3 | 13 | BaoStock 重连加锁 | 1h | 并发安全 |
| ✅ 已完成 | 🟢 P3 | 14 | 指标 TTL 配置化 | 1h | 可维护 |
| ✅ 已完成 | 🟢 P3 | 15 | 添加 CSP 响应头 | 30min | 安全 |
