...
class SessionConfig(BaseModel):
    connection_mode: Optional[Literal["per_call", "persistent"]] = Field(
        None,
        description="连接模式：per_call（每次调用独立登录/登出）| persistent（维持长连接+心跳）",
        example="per_call",
    )
    heartbeat_interval: Optional[int] = Field(
        None,
        description=f"心跳保活间隔（秒），persistent 模式生效，范围 {HEARTBEAT_MIN}–{HEARTBEAT_MAX}",
        ge=HEARTBEAT_MIN,
        le=HEARTBEAT_MAX,
        example=60,
    )
    idle_timeout: Optional[int] = Field(
        None,
        description=f"空闲自动登出阈值（秒），persistent 模式生效，范围 {IDLE_MIN}–{IDLE_MAX}",
        ge=IDLE_MIN,
        le=IDLE_MAX,
        example=600,
    )
...
|------|------|
| `logged_in` | 是否处于登录状态 |
| `connection_mode` | 当前连接模式（per_call / persistent） |
| `idle_seconds` | 距上次 API 调用的空闲秒数 |
| `idle_timeout` | 自动登出阈值（秒），persistent 模式生效 |
| `heartbeat_interval` | 心跳保活间隔（秒），persistent 模式生效 |
    """
)
def status():
    return get_status()
...
@router.patch(
    ...
    summary="修改会话配置",
    description=f"""
动态修改连接模式、心跳间隔和空闲超时设置，**立即生效**，无需重启服务。

| 参数 | 说明 | 可选值 / 范围 |
|------|------|------|
| `connection_mode` | 连接模式 | `per_call`（默认）/ `persistent` |
